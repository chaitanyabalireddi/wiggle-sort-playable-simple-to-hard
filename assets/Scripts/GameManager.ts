import { _decorator, Component, Node, input, Input, EventMouse, EventTouch, AudioSource, AudioClip, Camera, Prefab, instantiate, Label, tween, Vec3 } from 'cc';
...
export class GameManager extends Component {
    ...
    @property({ type: AudioClip, tooltip: 'Sound played when a snake enters the hole.' })
    ...
    @property({ type: [Number], tooltip: 'Camera ortho height per level. Index must match levelNodes. Leave empty to keep default.' })
    levelOrthoHeights: number[] = [];

    @property({ type: Prefab, tooltip: 'Particle effect spawned when a level is cleared' })
    levelClearParticlePrefab: Prefab = null;

    @property({ type: Label, tooltip: 'UI Label to display current level number' })
    levelLabel: Label = null;

    @property({ type: AudioClip, tooltip: 'Sound played when a level is cleared' })
    levelClearSfx: AudioClip = null;

    @property({ type: ConfettiManager, tooltip: 'Reference to the code-based confetti manager' })
    ...
    @property({ tooltip: 'On the last level, show CTA after this many snakes enter the hole (0 = wait for all)' })
    ...
    start() {
        ...
        this.node.scene.on('sfx-snake-enter-hole', this._onSnakeEnterHole, this);
    ...
    onDestroy() {
        ...
        this.node.scene.off('sfx-snake-enter-hole', this._onSnakeEnterHole, this);
    }
    ...
    private _activateLevel(index: number) {
        ...
        if (index < this.levelOrthoHeights.length && this.levelOrthoHeights[index] > 0) {
    ...
    private _findMainCamera(): Camera | null {
        const n = this.node.scene?.getChildByName('Camera')
    ...
    private _onSnakeDone() {
        ...
        if (isLastLevel && this.lastLevelCtaAfterSnakes > 0 && this._levelSnakesDone >= this.lastLevelCtaAfterSnakes) {
            this._triggerWin();
            return;
        }
    ...
    private _onLevelCleared() {
        ...
        if (nextLevel < this.levelNodes.length) {
            ...
            if (this.levelClearParticlePrefab) {
                ...
                this.scheduleOnce(() => { if (fx.isValid) fx.destroy(); }, 3.0);
            }
    ...
    private _showTutorialTarget() {
        ...
        const moveable = alive.filter(s => s.canMoveNow() && s.getActiveBlockers().length === 0);
        ...
        for (const s of alive) {
            if (moveable.indexOf(s) === -1) {
                ...
                console.log(`[Tutorial] NOT moveable: ${s.node.name}, color=${s.snakeColor}, canMove=${canMove}, blockers=${blockerCount}`);
            }
        }
        ...
        console.log(`[Tutorial] TIER 1: currentColor='${currentColor}', moveable count=${moveable.length}`);
        for (const s of moveable) {
            console.log(`[Tutorial]  moveable snake: name=${s.node.name}, color=${s.snakeColor}, blockers=${s.getActiveBlockers().length}`);
        }
        if (currentColor && moveable.length > 0) {
            const matching = moveable.filter(s =>
                s.snakeColor.toLowerCase().trim() === currentColor
            );
            console.log(`[Tutorial]  matching '${currentColor}' count=${matching.length}`);
        ...
        if (currentColor) {
            ...
            const blockedMatch = alive.filter(s =>
                s.snakeColor.toLowerCase().trim() === currentColor && s.canMoveNow()
            );
        ...
        if (nextColor && moveable.length > 0) {
            const matching = moveable.filter(s =>
                s.snakeColor.toLowerCase().trim() === nextColor
            );
            const best = pickClosest(matching);
            if (best) {
                this.tutorialHand.attachTo(best);
