...
export class SnakePath extends Component {
	...
	@property({
		type: CCFloat,
		tooltip:
			"Spline samples per segment for arc-length table. Higher = smoother but slower start.",
	})
	...
	private lookupArc(adjustedDist: number): Vec3 {
		...
		if (adjustedDist <= table[0].dist) {
			...
			const tFrac =
				table[0].dist > 0 ? e.t * (adjustedDist / table[0].dist) : 0;
		...
		while (lo < hi - 1) {
			const mid = (lo + hi) >> 1;
			if (table[mid].dist < adjustedDist) lo = mid;
			else hi = mid;
		}
