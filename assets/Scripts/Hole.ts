...
export class Hole extends Component {
    ...
    @property({ type: Node, tooltip: 'Empty child node at the exact visual center of the hole (optional)' })
    ...
    @property({ tooltip: 'If true, this hole acts as a visual preview for the main hole and does not accept snakes.' })
    ...
    start() {
        ...
        if (!this.holeRimRenderer) {
            this.holeRimRenderer = this.node.getComponent(MeshRenderer)
        ...
        if (this.colorMappings.length >= 2) {
            ...
            this.scheduleOnce(() => {
                ...
                this._currentColorName = this.colorMappings[0]?.colorName?.toLowerCase().trim() ?? '';
            ...
            this.scheduleOnce(() => {
                ...
                const active = snakes.filter((s: Snake) => s.node.activeInHierarchy && !s.isDone() && !s.isEnteringHole());
                if (active.length > 0) {
                    this._currentColorName = active[0].snakeColor.toLowerCase().trim();
                    this.applyColorToRenderer(this.holeRimRenderer, this._currentColorName);
                    if (active.length > 1) {
                        // Find a different color for the next indicator
                        const different = active.find(s => s.snakeColor.toLowerCase().trim() !== this._currentColorName);
                        if (different) {
                            this._nextColorName = different.snakeColor.toLowerCase().trim();
                            this.applyColorToRenderer(this.nextIndicatorRenderer, this._nextColorName);
                        } else {
                            this._nextColorName = '';
    ...
    private colorNameToColor(name: string): Color {
        switch (name.toLowerCase().trim()) {
            ...
            default:       return new Color( 40,  40,  40, 255);
        }
    }
    ...
    updateMaterials() {
        const currentColorName = this.colorMappings[this._currentIndex]?.colorName ?? '';
        ...
        if (this.nextIndicatorRenderer) {
            if (this._nextIndex === -1) {
                ...
                const nextColorName = this.colorMappings[this._nextIndex]?.colorName ?? '';
                ...
                this.applyColorToRenderer(this.nextIndicatorRenderer, nextColorName);
            }
        }
    }
    ...
    advanceColor() {
        ...
        if (this.colorMappings.length >= 2) {
            ...
            this._currentColorName = this.colorMappings[this._currentIndex]?.colorName?.toLowerCase().trim() ?? '';

            if (active.length > 0) {
                const hasMatch = active.some(s =>
                    s.snakeColor.toLowerCase().trim() === this._currentColorName
                );
                if (!hasMatch) {
            ...
            this._nextIndex = this.getBestCandidateIndex();
            this._nextColorName = this._nextIndex !== -1
                ? (this.colorMappings[this._nextIndex]?.colorName?.toLowerCase().trim() ?? '')
                : '';
            ...
            if (active.length > 0) {
                ...
                if (levelRoot) {
                    ...
                    for (const h of allHoles) if (h.node !== this.node) otherHoles.push(h);
                }
                ...
                if (easiestIdx !== -1) {
                    const easiestColor = this.colorMappings[easiestIdx]?.colorName?.toLowerCase().trim();
                    bestMatch = active.find(s => s.snakeColor.toLowerCase().trim() === easiestColor);
                }
                // If no easiest match found or it's used by other holes, try to find any unused
                if (!bestMatch) {
                    bestMatch = active.find(s => otherColors.indexOf(s.snakeColor.toLowerCase().trim()) === -1);
                }
                ...
                if (newColor === this._currentColorName && active.length > 1) {
                    const different = active.find(s => s.snakeColor.toLowerCase().trim() !== this._currentColorName);
                    if (different) {
                        this._currentColorName = different.snakeColor.toLowerCase().trim();
                    } else {
                ...
                if (nextEasiestIdx !== -1) {
                    const nextEasiestColor = this.colorMappings[nextEasiestIdx]?.colorName?.toLowerCase().trim();
                    nextMatch = active.find(s => s.snakeColor.toLowerCase().trim() === nextEasiestColor);
                }
                if (!nextMatch) nextMatch = active.find(s => s.snakeColor.toLowerCase().trim() !== this._currentColorName);
    ...
    private getEasiestSnakeIndex(): number {
        ...
        type Candidate = { colorIndex: number; color: string; easeScore: number; dist: number };
        ...
        for (const snake of snakes) {
            ...
            candidates.push({ colorIndex: mappingIdx, color: snakeCol, easeScore, dist });
        }
    ...
    getCenterWorldPosition(): Vec3 {
        return this.holeCenter ? this.holeCenter.getWorldPosition() : this.node.getWorldPosition();
    }
    ...
    private _getActiveSnakes(allSnakes: Snake[]): Snake[] {
        return allSnakes.filter(s => s.node.activeInHierarchy && !s.hasFailed() && !s.isEnteringHole() && !s.isEntryFinished());
    }
    ...
    private _forceCurrentColorToSnake(snake: Snake): void {
        ...
        if (snakeCol === currentCol) {
            ...
            const different = active.find(s => s.snakeColor.toLowerCase().trim() !== currentCol);
            if (different) {
    ...
    private _forceCurrentColorToSnakeInternal(snake: Snake): void {
        ...
        if (this.colorMappings.length >= 2) {
            ...
            if (idx !== -1) {
                if (this._currentIndex === idx && this._nextIndex === -1 && this._currentColorName === snakeCol) return;
                this._currentIndex = idx;
                this._currentColorName = snakeCol;
                this._nextIndex = this.getBestCandidateIndex();
                this.updateMaterials();
            ...
            const remaining = this.node.scene.getComponentsInChildren(Snake)
                .filter(s => s.node.activeInHierarchy && !s.isDone() && !s.isEnteringHole() && s !== snake);
            
            const different = remaining.find(s => s.snakeColor.toLowerCase().trim() !== snakeCol);
    ...
    private _handlePreviewLogic(): void {
        ...
        const allHoles = levelRoot.getComponentsInChildren('Hole') as any[];
        const mainHole = allHoles.find(h => h.node !== this.node && !h.isNextPreview);
        
        if (mainHole) {
            const nextIdx = mainHole._nextIndex;
            const nextCol = mainHole._nextColorName;
            
            if (this._currentIndex !== nextIdx || this._currentColorName !== nextCol) {
    ...
    private findLevelRoot(): Node | null {
        ...
        while (n) {
            ...
            if (n.parent && n.parent.name.toLowerCase().indexOf('scene') !== -1) return n;
