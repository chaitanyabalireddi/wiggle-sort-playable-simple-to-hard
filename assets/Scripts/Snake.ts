import { _decorator, Component, Node, Vec3, instantiate, Prefab, Quat, math, input, Input, EventMouse, EventTouch, geometry, Camera, MeshRenderer, Color, ParticleSystem } from 'cc';
...
export class Snake extends Component {
    ...
    @property({ type: Node, tooltip: 'Empty node placed in the scene to define which direction this snake flies when tapped' })
    directionTarget: Node = null;

    @property({ tooltip: 'Minimum world-unit gap to detect collision with another snake' })
    collisionRadius: number = 0.6;

    @property({ tooltip: 'Distance from origin before snake is considered off-screen' })
    offScreenDistance: number = 20.0;

    @property({ type: [Node], tooltip: 'Snake nodes that must move out first before this snake can move. Assign the Snake root nodes here.' })
    blockedBy: Node[] = [];

    @property({ type: SnakePath, tooltip: 'Path this snake follows when tapped. Leave empty for straight-line movement.' })
    ...
    @property({ tooltip: 'How close the head must be to a hole to start seeking it' })
    holeSeekRadius: number = 5.0;

    @property({ tooltip: 'Distance to keep from other snakes when waiting for path to clear' })
    pathClearanceRadius: number = 4.0;

    @property({ tooltip: 'Minimum gap required before resuming movement after waiting' })
    ...
    start() {
        ...
        if (this.snakePath) {
            const headPos = this.headNode ? this.headNode.getWorldPosition() : this.node.getWorldPosition();
    ...
    private seedHistoryFromPose() {
        ...
        if (this.headNode) posePoints.push(this.headNode.getWorldPosition().clone());
        for (const seg of this.bodySegments) {
        ...
        for (let i = 1; i < posePoints.length; i++) {
            poseDist.push(poseDist[i - 1] + Vec3.distance(posePoints[i - 1], posePoints[i]));
        }
    ...
    private samplePoseChain(points: Vec3[], dists: number[], targetDist: number): Vec3 {
        if (targetDist <= 0) return points[0].clone();
        for (let i = 1; i < points.length; i++) {
            if (dists[i] >= targetDist) {
                const segLen = dists[i] - dists[i - 1];
                const t = segLen > 0.0001 ? (targetDist - dists[i - 1]) / segLen : 0;
                const out = new Vec3();
    ...
    public getActiveBlockers(): Snake[] {
        ...
        for (const blockerNode of this.blockedBy) {
            if (!blockerNode || !blockerNode.isValid || !blockerNode.active) continue;
    ...
    private isOverlapping(nodesA: Node[], nodesB: Node[]): boolean {
        for (const a of nodesA) {
            ...
            for (const b of nodesB) {
                ...
                const dx = pa.x - pb.x;
                const dz = pa.z - pb.z;
                if (dx * dx + dz * dz < this.collisionRadius * this.collisionRadius) {
                    return true;
                }
            }
        }
    ...
    public startBlockedShake() {
        ...
        for (const n of nodes) {
            this._shakeOriginPositions.push({ node: n, pos: n.getWorldPosition().clone() });
        }
    }
    ...
    update(deltaTime: number) {
        ...
        if (this._isShaking) {
            ...
            if (this._shakeTimer >= this._shakeDuration) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    const offset = Math.sin(this._shakeTimer * this._shakeFrequency - segPhase) * amplitude * tailScale;
        ...
        if (this._isEnteringHole) {
            ...
            if (distToHole > 0.01) {
                ...
                newHeadPos = new Vec3();
                Vec3.scaleAndAdd(newHeadPos, headCurrentPos, toHole, Math.min(step, distToHole));
            } else {
            ...
            const headDistToHole = Vec3.distance(newHeadPos, this._enterHolePos);
            const headShrinkStart = 1.5;
            const headScale = Math.max(0.01, Math.min(1, headDistToHole / headShrinkStart));
            this.headNode.setScale(headScale, headScale, headScale);
            ...
            for (let i = 0; i < this.bodySegments.length; i++) {
                ...
                const prevPos = seg.getWorldPosition();
                seg.setWorldPosition(segPos);
                this.faceMovementDirection(seg, prevPos, segPos);

                const distToHole = Vec3.distance(segPos, this._enterHolePos);
                const shrink = Math.max(0.05, Math.min(1, distToHole / 1.5));
                seg.setScale(shrink, shrink, shrink);

                if (distToHole > 0.2) allAtHole = false;
            }

            this._enterHoleProgress += deltaTime / this._enterHoleDuration;

            if (allAtHole || this._enterHoleProgress >= 3.0) {
        ...
        if (this._isBouncing) {
            ...
            for (let i = 0; i < nodes.length; i++) {
                ...
                if (i < this._bounceRotations.length && i < this._originRotations.length) {
                    const r = new Quat();
                    Quat.slerp(r, this._bounceRotations[i], this._originRotations[i], t);
            ...
            if (this._bounceProgress >= 1.0) {
                ...
                for (let i = 0; i < nodes.length && i < this._originPose.length; i++) {
                    nodes[i].setWorldPosition(this._originPose[i]);
                    if (i < this._originRotations.length) nodes[i].setWorldRotation(this._originRotations[i]);
                }
                this.seedHistoryFromPose();
            }
            return;
        ...
        if (this._seekingHole) {
                ...
                if (hole?.entryParticlePrefab) {
                    ...
                    setTimeout(() => {
                        ...
                        const ps2 = fx.getComponent(ParticleSystem)
                                  ?? fx.getComponentInChildren(ParticleSystem);
                        ps2?.stop();
                        setTimeout(() => { if (fx?.isValid) fx.destroy(); }, 1000);
            ...
            const currentTarget = this.snakePath.getPointAtDistance(this._pathDistance);
            ...
            if (distToPath > 0.1) {
                ...
                Vec3.scaleAndAdd(candidatePos, prevHeadPos, toTarget, Math.min(step, distToPath));
                ...
                if (hitIdleSnake && !hitIdleSnake._isMoving) {
                    ...
                    return;
                } else if (this.isBlockedByOnPathSnake(candidatePos) || hitIdleSnake) {
                ...
                const bodyLength = this.bodySegments.length * this.segmentSpacing + 1.0;
                ...
                if (this.mustYieldToHigherPrioritySnake(prevHeadPos)) {
                    ...
                    if (pathLen > 0) {
                        this._pathDistance = ((this._pathDistance % pathLen) + pathLen) % pathLen;
                    }
                    newHeadPos = this.snakePath.getPointAtDistance(this._pathDistance);
    ...
    private checkCollisionWithOthers(headPos: Vec3): Snake | null {
        ...
        for (const other of allSnakes) {
            ...
            for (const seg of other.bodySegments) if (seg?.active) otherNodes.push(seg);
    ...
    private hasFullyEnteredPath(): boolean {
        ...
        const pathLen = this.snakePath.getPathLength();
        const entryPoint = this.snakePath.getPointAtDistance(this._pathEntryDistance);
        ...
        for (const node of this.getAllNodes()) {
            ...
            const minPathDist = pathLen > 0 ? Math.min(distFromEntry, pathLen - distFromEntry) : distFromEntry;
        ...
        for (const other of allSnakes) {
            ...
            if (thisOnPath && otherOnPath) {
                ...
                if (this._pathTraveled > other._pathTraveled) {
                    ...
                    const pathLen = this.snakePath.getPathLength();
                    const distAlongPath = Math.abs(this._pathDistance - other._pathDistance);
                    const minPathGap = pathLen > 0 ? Math.min(distAlongPath, pathLen - distAlongPath) : distAlongPath;
                ...
                if (other._pathTraveled > this._pathTraveled && this._pathEntryDistance >= 0) {
                    const otherDistFromMyEntry = Math.abs(other._pathDistance - this._pathEntryDistance);
                    const pathLen = this.snakePath.getPathLength();
                    const minDist = pathLen > 0 ? Math.min(otherDistFromMyEntry, pathLen - otherDistFromMyEntry) : otherDistFromMyEntry;
    ...
    isMovingNow(): boolean  { return this._isMoving || this._isBouncing; }
    canMoveNow(): boolean   { return !this._done && !this._isMoving && !this._isBouncing; }
    getHeadNode(): Node | null { return this.headNode; }
    isSeekingHole(): boolean { return this._seekingHole; }

    hasFailed(): boolean       { return false; }
    isEnteringHole(): boolean  { return this._isEnteringHole; }
    isEntryFinished(): boolean { return this._done; }
    isLocked(): boolean        { return false; }
    isFrontBlocked(): boolean  { return false; }
    ...
    private _checkForNearbyHole(headPos: Vec3) {
        ...
        for (const holeNode of holeNodes) {
            ...
            const isMatch = myColor !== '' && holeColor !== '' && (
                myColor === holeColor
                || holeColor.includes(myColor)
                || myColor.includes(holeColor));
            if (!isMatch) continue;

            const centerPos: Vec3 = hole.getCenterWorldPosition?.()
                ?? holeNode.getWorldPosition();
            const diff = new Vec3();
    ...
    private _triggerHoleEntry(holeNode: Node) {
        ...
        const hole = holeNode.getComponent('Hole') as any;
        const holePos: Vec3 = hole?.getCenterWorldPosition?.() ?? holeNode.getWorldPosition();
        ...
        this.enterHole(holePos, () => {
            ...
            if (hole && (!hole.colorMappings || hole.colorMappings.length < 2)) {
                const remaining = this.node.scene.getComponentsInChildren(Snake)
                    .filter((s: Snake) => s.node.activeInHierarchy && !s.isDone() && !s.isEnteringHole());
                if (remaining.length > 0 && hole.applyColorToRenderer) {
                    hole._currentColorName = remaining[0].snakeColor.toLowerCase().trim();
                    hole.applyColorToRenderer(hole.holeRimRenderer, hole._currentColorName);
                    if (remaining.length > 1) {
                        hole._nextColorName = remaining[1].snakeColor.toLowerCase().trim();
                        hole.applyColorToRenderer(hole.nextIndicatorRenderer, hole._nextColorName);
                    }
                }
            }
        });
